...
if [ "${H3_NIGHTLY_SKIP_GENERATE:-0}" != "1" ]; then
  stage "① 全自动流水线（生成→评分→分析→Seed）" \
    python3 scripts/pipeline-tools/pipeline_auto.py --auto "${BATCH_ARGS[@]+"${BATCH_ARGS[@]}"}"
fi
...
stage "④ 面板数据刷新" python3 scripts/pipeline-tools/export_dashboard_data.py

# 收尾：raw 日志并入报告，展示摘要
[ -f "$REPORT.raw" ] && { echo "" && echo "<details><summary>完整日志</summary>" && echo "" && echo '```' && cat "$REPORT.raw" && echo '```' && echo "</details>" >> "$REPORT" && rm -f "$REPORT.raw"; }
